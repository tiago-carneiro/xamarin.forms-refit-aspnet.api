        ...
        static ApiResult<T> Error<T>(ApiException apiEx)
        {
            switch (apiEx.StatusCode)
            {
                case HttpStatusCode.BadRequest:
                    return ApiResult.Create(default(T), false, apiEx.StatusCode, JsonConvert.DeserializeObject<JObject>(apiEx.Content).Value<string>("Message"));
                case HttpStatusCode.NotFound:
                    return ApiResult.Create(default(T), false, apiEx.StatusCode, "Não encontrado");

                case HttpStatusCode.InternalServerError:
                default:
                    return ApiResult.Create(default(T), false, apiEx.StatusCode, "Ocorreu um erro inesperado");
            }
        }

        static ApiResult<T> Error<T>(Exception ex) =>
            ApiResult.Create(default(T), false, HttpStatusCode.ExpectationFailed, "Ocorreu um erro inesperado");
    }
}
